...
async function loadPokemon() {
    for (let i = start; i < limit; i++) {                                     // starte bei eins und höre auf bei 19
        ...
        fetchedPokemons.push(currentPokemon);                                 // push das ganze in ein neues Array rein(fetchedPokemons)
    }
    start += 20;                                                              // erhöhe den Wert um 20 wenn die Funktion zum Zweiten Mal durchgeführt wird 
    limit += 20;                                                              // erhöhe den Wert um 20 wenn die Funktion zum Zweiten Mal durchgeführt wird
    loadedPokemons = fetchedPokemons;                                         // loadedPokemons ist das gleiche Array wie fetchedPokemons
...
function renderCards() {
    ...
    for (let i = 0; i < loadedPokemons.length; i++) {                                            // iteriere durch die loadedPokemons Array
...
// get the Stats for the Chart from the loadedPokemons Array
function getChartStats(i) {
    let statsName = [];                           //in Case there would variable names
    let statsPower = [];
    ...
    for (let s = 0; s < stats.length; s++) {
        ...
        statsName.push(myStats['stat']['name'].toUpperCase());   //in Case there would variable names
...
function searchPokemon() {
    ...
    // loadedPokemons enthält selber all die Pokemons die in fetchedPokemons drin sind 
    // also fetchedPokemons.length = 40, loadedPokemons.length = 40
    // und wenn die Zeile drunter ausgeführt wird, wird loadedPokemons überschrieben 
    loadedPokemons = fetchedPokemons.filter(pokemon => pokemon.name.includes(searchInput)); // callback funktion, loadedPokemons ist jetzt neues Array mit all den Elmenten die man in das inputfeld eintippt
